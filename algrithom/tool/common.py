...
class ResultProcess:
    ...
    def mechine_result_process(warn_detect,frame,param,warn_flag,timestamp):
        ...
        msg["image"]=frame
        msg["warnflag"]=warn_flag
        for mechine, info in msg["results"]["statitic"].items():
            if isinstance(info['track_ids'], set):
                info['track_ids'] = list(info['track_ids'])
        return msg
import  os
def save_data(algorithm_type,img,timestamp):
    try:
        save_path="data/detect_data_save/{}".format(algorithm_type)
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        cv2.imwrite("data/detect_data_save/{}/{}.jpg".format(algorithm_type,timestamp),img)
