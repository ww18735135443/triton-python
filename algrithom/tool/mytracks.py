...
class Tracks:
    ...
    def wearupdate(self,detections,wearattr='belt'):
        ...
        if wearattr=='lifejacket':
            for track in self.tracks:
                is_wear=0
                for detection in detections:
                    if detection['cls']=='lifejacket':
                        wear_xyxy=detection['xyxy']
                        ioua=calculate_ioua(wear_xyxy, track.current_xyxy)
                        if ioua>track.attrThreshold:
                            track.lifejacket.appendleft(1)
                            is_wear=1
                            break
                if is_wear==0:
                    track.lifejacket.appendleft(0)
                if sum(i for i in track.lifejacket)/len(track.lifejacket)<0.1 and len(track.lifejacket)>10:
                    track.wearattr[wearattr]=0
                else:
                    track.wearattr[wearattr]=2
                while len(track.lifejacket)>25:
                    track.lifejacket.pop()