...
class AlgThread:
    ...
    def sendkafkamsg(self,msg):
        if self.msgapp.kafka_send:
            msg["image"]=img_2_base64(msg["image"])
            msg = convert_ndarray_to_list(msg)
            topic_name =self.param['topic']
            # # 发送报警事件到Kafka
            self.logger.info("开始发送消息,topic_name:{}".format(topic_name))

            future = self.msgapp.producer.send(topic_name, msg)
            # 尝试获取发送结果，同时处理可能的异常
            try:
                # 等待消息发送完成（或直到超时），这里假设超时时间为30秒
                ...
                # 处理发送过程中出现的异常
                self.logger.info('Failed to send message: {}'.format(e))
        else:
    ...
    def send_message(self,msg,vl_check=0):
        try:
            save_path="data/detect_data/{}".format(msg['algorithm_type'])
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            cv2.imwrite("data/detect_data/{}/{}.jpg".format(msg['algorithm_type'],msg["results"]["info"]["timestamp"]),msg["image"])
            self.logger.info("数据回流成功")
        except Exception as e:
            self.logger.info("数据回流失败")
        if vl_check:
            self.msgapp.send(msg,msg['algorithm_type'])
        else:
