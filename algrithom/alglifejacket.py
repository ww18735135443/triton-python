...
def result_process(warn_detect,frame,param,warn_flag,timestamp):
    ...
    return msg
class LifejacketAlgThread(threading.Thread,AlgThread):
    def __init__(self,param):
        ...
        self.logger.info('救生衣检测算法线程初始化成功！')
        ...
        for target in targets:
            ...
            curThread.start()
        self.logger.info('救生衣检测算法线程启动成功')
    ...
    def run(self):
        while True:
            try:
                ...
                if len(warn_object)>0:
                    self.logger.info("taskid:{},warn_flag:{}, timestamp:{},warn_object:{}".format(self.param['interfaceId'],warn_flag,timestamp,warn_object))
